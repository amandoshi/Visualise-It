...
class DFS {
	...
	*#run(prevNode=null) {
		if (!this.#unvisitedNodes.isEmpty() && !this.#targetFound) {
			const currentNode = this.#unvisitedNodes.elements[0];
			const neighbourNodes = this.#graph.neighbours(currentNode);

			if (prevNode != null) {
				const data = {
					node: currentNode,
					edge: this.#graph.connection(prevNode, currentNode),
				};
				yield data
			} else {
				yield
			...
			this.#unvisitedNodes.removeItem();
